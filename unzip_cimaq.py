...
def unzip_cimaq(cimaq_dir:Union[os.PathLike, str], keepinfos:bool=False, keepsheets:bool=False):
    ...
    zip_contents = [scan_zip_contents(fpath, exclude = ['pratique', 'practice'],
                                   to_xtrct = ['.pdf', '.edat2'],
                                   withbytes=True,
    ...
    if keepsheets and keepinfos:
        return tuple(zip([row[1] for row in
                          sheet_infos.sort_values('filename').iterrows()],
                         sorted(newsheets)))
