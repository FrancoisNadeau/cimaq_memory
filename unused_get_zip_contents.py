...
def get_zip_contents(
    ...
    vals[["filename", "src_name", "ext", "bsheets"]] = \
    
        [(ntpl, [os.path.splitext(nm)[1]]*len(ntpl),
          "_".join(pd.Series(row[1].filename.lower().replace(
              "/", "_").replace("-", "_").split("_")).unique().__iter__()),
         myzip.open(row[1].src_name).read().lower())
         for row in vals.iterrows()]
    myzip.close()

    if exclude:
        vals = vals.drop(
            [row[0] for row in vals.iterrows()
             if row[1].filename not in
             filter_lst_exc(exclude,
                            [itm.lower() for itm in
                             vals.filename])],axis=0,)
